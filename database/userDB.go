...
func (p *PostgreStorage) CreateUser(user models.User) (int64, error) {
	// Проверяем существование пользователя по логину
	var existingUserID int64
	err := p.db.QueryRow("SELECT id FROM users WHERE login = $1", user.Login).Scan(&existingUserID)
	if err == nil {
	...
	var userID int64
	err = p.db.QueryRow("INSERT INTO users (login, password, created_at) VALUES ($1, $2, $3) RETURNING id", user.Login, user.Password, user.CreatedAt).Scan(&userID)
	if err != nil {
